/* ═══════════════════════════════════════════════
   FixMate - פרופיל בעל מקצוע (ProProfile)
   הצגה ועריכה של פרטי המקצוען: שם, טלפון, מייל, מחירים, אזורי שירות, דירוגים
   ═══════════════════════════════════════════════ */
...
import { getLang, getDir } from "../context/LanguageContext";  // כלי שפה וכיוון
...
// מצלמה - להחלפת תמונת פרופיל
const IconCamera = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" /><circle cx="12" cy="13" r="4" />
  </svg>
);
// חץ שברון (מסתובב למעלה/למטה לפי פתוח/סגור)
...
/* ═══════════════════════════════════════════════
   נתונים - פרטי המקצוען והאזורים
   ═══════════════════════════════════════════════ */
const SHEKEL = "\u20AA";   // סימן שקל ₪

// ─── פרטי ברירת המחדל של המקצוען (נטענים מהשרת בעתיד) ───
const INIT = {
  name: { en: "Amoona Hiouala", he: "\u05D0\u05DE\u05D5\u05E0\u05D4 \u05D7\u05D9\u05D5\u05D0\u05DC\u05D4" },   // שם מלא בשתי שפות
  role: { en: "Licensed Electrician", he: "\u05D7\u05E9\u05DE\u05DC\u05D0\u05D9 \u05DE\u05D5\u05E1\u05DE\u05DA" },   // המקצוע
  avatar: { en: "AM", he: "\u05D0\u05D7" },        // ראשי תיבות לאווטאר
  rating: 4.84,                                      // דירוג ממוצע
  reviewCount: 127,                                  // מספר ביקורות
  phone: "054-885-8793",                            // טלפון
  email: "amore@gmail.com",                         // אימייל
  bio: { en: "Experienced professional with 12 years of experience in the field.", he: "\u05D1\u05E2\u05DC \u05DE\u05E7\u05E6\u05D5\u05E2 \u05DE\u05E0\u05D5\u05E1\u05D4 \u05E2\u05DD 12 \u05E9\u05E0\u05D5\u05EA \u05E0\u05D9\u05E1\u05D9\u05D5\u05DF" },   // תיאור עצמי
  minPrice: 150,                                     // מחיר מינימום בש"ח
  maxPrice: 1000,                                    // מחיר מקסימום בש"ח
  areas: [                                           // אזורי שירות נוכחיים
...
// ─── רשימת כל האזורים האפשריים (להוספת אזור חדש) ───
const ALL_AREAS = [
  { en: "Tel Aviv", he: "\u05EA\u05DC \u05D0\u05D1\u05D9\u05D1" },
  { en: "Rishon LeZion", he: "\u05E8\u05D9\u05E9\u05D5\u05DF \u05DC\u05E6\u05D9\u05D5\u05DF" },
  { en: "Bat Yam", he: "\u05D1\u05EA \u05D9\u05DD" },
...
/* ═══════════════════════════════════════════════
   הקומפוננטה הראשית - דף פרופיל מקצוען
   ═══════════════════════════════════════════════ */
export default function ProProfile() {
  ...
  const isRTL = dir === "rtl";                       // האם RTL?
  // פונקציה שולפת ערך בשפה הנכונה מאובייקט דו-שפתי
  ...
  const [editing, setEditing] = useState(false);                 // האם במצב עריכה?
  const [saved, setSaved] = useState(false);                     // האם הופיעה הודעת "נשמר"?
  const [showAreaPicker, setShowAreaPicker] = useState(false);   // האם תפריט בחירת אזור פתוח?
  const [showAllReviews, setShowAllReviews] = useState(false);   // האם להציג את כל הביקורות?

  /* ─── משתני מצב של הטופס ─── */
  const [fullName, setFullName] = useState(L(INIT.name));   // שם מלא
  const [phone, setPhone] = useState(INIT.phone);            // טלפון
  const [email, setEmail] = useState(INIT.email);            // אימייל
  const [bio, setBio] = useState(L(INIT.bio));               // תיאור עצמי
  const [minPrice, setMinPrice] = useState(INIT.minPrice);   // מחיר מינימום
  const [maxPrice, setMaxPrice] = useState(INIT.maxPrice);   // מחיר מקסימום
  const [areas, setAreas] = useState(INIT.areas);            // רשימת אזורי שירות

  /* ─── אנימציית כניסה ─── */
  useEffect(() => { setTimeout(() => setMounted(true), 50); }, []);

  // פונקציית חזרה לדשבורד
  const goBack = () => navigate("/pro/dashboard");

  /* ─── שמירת שינויים ─── */
  const handleSave = () => {
    setEditing(false);                                 // יוצא ממצב עריכה
    setSaved(true);                                    // מציג הודעת "נשמר"
    setTimeout(() => setSaved(false), 2500);           // מסתיר אחרי 2.5 שניות
  };

  // הסרת אזור מרשימת האזורים
  const removeArea = (idx) => setAreas((prev) => prev.filter((_, i) => i !== idx));

  // הוספת אזור חדש (אם לא קיים כבר)
  const addArea = (area) => {
    if (!areas.find((a) => a.en === area.en)) setAreas((prev) => [...prev, area]);
    setShowAreaPicker(false);                           // סוגר את התפריט
  };

  /* ─── הגדרות פונטים ─── */
  const font = isHe ? "'Heebo',sans-serif" : "'DM Sans',sans-serif";    // פונט לפי שפה
  const titleFont = "'Outfit',sans-serif";                                // פונט לכותרות
  ...
  /* ─── סגנון שדה קלט (פעיל = גבול כחול, לא פעיל = אפור) ─── */
  ...
  // אם showAllReviews=true - מציג הכל, אחרת - רק 2 ראשונות
    ...
    <div style={{ fontFamily: font, background: "#F5F7FB", minHeight: "100vh", direction: dir, textAlign: isRTL ? "right" : "left", opacity: mounted ? 1 : 0, transition: "opacity .35s" }}>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(255,255,255,.96)", backdropFilter: "blur(16px)", borderBottom: "1px solid #E8ECF4" }}>
        <div className="pro-nav-inner" style={{ maxWidth: 960, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 28px" }}>
          {/* כפתור חזרה - חץ שמשתנה לפי כיוון השפה */}
          ...
          {/* אם לא במצב עריכה - מציג כפתור "ערוך" */}
          {!editing ? (
            ...
            /* אם במצב עריכה - מציג 2 כפתורים: ביטול + שמור */
            <div style={{ display: "flex", gap: 8 }}>
              {/* כפתור ביטול - יוצא ממצב עריכה ללא שמירה */}
      ...
      {/* ═══ הודעת Toast - "נשמר בהצלחה" (מופיעה ל-2.5 שניות) ═══ */}
      {saved && (
      ...
      <div className="pro-wrap" style={{ maxWidth: 960, margin: "0 auto", padding: "28px 28px 48px" }}>
        ...
        {/* ═══ HERO - כרטיס ראשי עם אווטאר + שם + דירוג ═══ */}
        <div style={{ ...card, marginBottom: 24, animation: "fadeUp .35s" }}>
          <div className="pro-hero-flex" style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap" }}>
            {/* ─── אווטאר (ראשי תיבות) + כפתור החלפת תמונה (רק בעריכה) ─── */}
            <div className="pro-hero-avatar" style={{ position: "relative" }}>
              <div style={{ width: 88, height: 88, borderRadius: 20, background: "linear-gradient(135deg,#2563EB,#1D4ED8)", color: "#FFF", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 26, fontFamily: titleFont, boxShadow: "0 6px 20px rgba(37,99,235,.22)" }}>
                {L(INIT.avatar)}     {/* ראשי תיבות בשפה הנכונה */}
              </div>
              {/* כפתור מצלמה קטן - מוצג רק במצב עריכה */}
              {editing && (
            ...
            {/* ─── פרטי המקצוען: שם, מקצוע, דירוג ─── */}
            <div className="pro-hero-info" style={{ flex: 1, minWidth: 180 }}>
              <h1 style={{ fontFamily: titleFont, fontSize: 24, fontWeight: 800, color: "#1A2B4A", marginBottom: 3 }}>{fullName}</h1>     {/* שם מלא */}
              <p style={{ fontSize: 14, color: "#2563EB", fontWeight: 600, marginBottom: 10 }}>{L(INIT.role)}</p>                       {/* המקצוע */}
              {/* שורת דירוג: כוכב + ציון + מספר ביקורות */}
        ...
        <div className="pro-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 24 }}>
          ...
          {/* ─── טור ימין: 2 כרטיסים - טווח מחירים + דירוג ─── */}
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ ...card, animation: "fadeUp .45s" }}>
              ...
              {/* קופסה עם הטווח המלא (תצוגה בלבד) */}
            ...
            <div style={{ ...card, animation: "fadeUp .5s" }}>
              ...
              {/* ─── שורה עליונה: ציון גדול + טקסט "X ביקורות מאומתות" ─── */}
              <div style={{ display: "flex", alignItems: "center", gap: 18, marginBottom: 20 }}>
                {/* ציון ממוצע גדול + 5 כוכבים */}
        ...
        <div style={{ ...card, animation: "fadeUp .55s" }}>
          ...
          {/* ─── תגיות אזורים נבחרים + כפתור הוספה (בעריכה) ─── */}
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 20 }}>
            {/* כל אזור = תגית עם אייקון + שם + כפתור הסרה (בעריכה) */}
            {areas.map((area, i) => (
            ...
            {/* כפתור "+ הוסף אזור" - מוצג רק במצב עריכה */}
            {editing && (
          ...
          {/* ─── תפריט בחירת אזור (מוצג רק כש-showAreaPicker=true) ─── */}
          {showAreaPicker && (
            <div style={{ background: "#FFF", borderRadius: 14, border: "1px solid #E8ECF4", boxShadow: "0 6px 24px rgba(0,0,0,.08)", padding: 6, marginBottom: 20, maxWidth: 300, animation: "pop .2s" }}>
              {/* מציג רק אזורים שעדיין לא נבחרו */}
              {ALL_AREAS.filter((a) => !areas.find((x) => x.en === a.en)).map((area, i) => (
                <button key={i} onClick={() => addArea(area)} style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "none", background: "transparent", textAlign: isRTL ? "right" : "left", fontSize: 14, color: "#1A2B4A", cursor: "pointer", display: "flex", alignItems: "center", gap: 7, fontFamily: font }}
                  onMouseEnter={(e) => { e.currentTarget.style.background = "#F0F4FF"; }}   // רקע מרחף
                  onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}>
                  <IconMapPin /> {L(area)}
                </button>
              ))}
              {/* הודעה אם כל האזורים כבר נבחרו */}
          ...
          {/* ─── Placeholder של מפה (Google Maps - להוספה בעתיד) ─── */}
